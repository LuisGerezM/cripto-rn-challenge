...
export const useCriptoList = () => {
  ...
  const [lodingCriptosByUser, setLodingCriptosByUser] = useState(true);
  const [errorMessage, setErrorMessage] = useState<string>('');

  const fetchCriptosByUser = async (signal: AbortSignal) => {
    try {
  ...
  useEffect(() => {
    if (criptoData.length > 0) {
      ...
      setErrorMessage('');
      fetchCriptosByUser(controller.signal);
  ...
  return {lodingCriptosByUser, criptoData, errorMessage};
};
